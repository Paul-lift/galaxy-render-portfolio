...
export default function SkillItem({
  ...
  const { displayedText: skillName, setShouldStart: setSkillNameStart } =
    useTypewriter(skill.name, 30, categoryIndex * 75 + skillIndex * 400 + 4000);
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center">
        ...
        <motion.span
          className="text-xs text-muted-foreground px-2 py-1 bg-primary/10 rounded-md border border-primary/20"
          animate={
            showTags ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.8 }
          }
        >
          {skill.level}
        </motion.span>
